...
https://github.com/Chris00/ocaml-csv/blob/master/examples/example.ml

ocamlbuild csv_1.ml csv_1.native -pkg csv
*)
...
let csvs = List.map (fun name -> name, Csv.load name) [ "addresses.csv";]
let csvs_ = List.map (fun name -> name, Csv.load name) [ "addresses.csv";]
